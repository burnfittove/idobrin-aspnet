...
public class UserController(IUserService userService, IConfiguration config) : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> UpdateUserRole(int id, UserUpdateRole user,
        CancellationToken cancellationToken = default)
    {
        // Check for a mismatch between the provided id and User
        if (id != user.id) return BadRequest("ID mismatch");
        
        // Update that user's role
        var result = await _userService.UpdateRoleAsync(id, user, cancellationToken);
